...
if ~isempty(hBd); handles(end+1) = hBd; labels{end+1} = 'BoL OCP'; end

% --- EoL C/50
[hEd, ~] = localPlotC50Phase(ax, eolC50, colEol);
if ~isempty(hEd); handles(end+1) = hEd; labels{end+1} = 'EoL OCP'; end
...
if ~isempty(handles)
    legend(ax, handles, labels, 'Location','best', 'Box','off', ...
        'FontName', fontName, 'FontSize', fontSize-1, 'NumColumns', 2);
else
...
h = plot(ax, gittEp.cumQ_signed_Ah, gittEp.OCV_V, 'o', ...
...
if ~isempty(c50.chargeQ_Ah)
    hCharge = plot(ax, c50.chargeQ_Ah, c50.chargeV, '--', ...
        'Color', col, 'LineWidth', 0.9, 'HandleVisibility','off');
end
end
