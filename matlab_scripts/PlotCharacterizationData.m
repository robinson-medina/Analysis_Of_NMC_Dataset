...
% Summary: Plots all the characterization data from the CSV files in the
% specified characterization folder and builds the combined multi-phase
% characterisation figure (Initialization, GITT, CC cycles, dynamic cycle,
% HPPC) used in the paper, with one subplot per data column.
%
% Usage:   Update the variable 'dataFolder' with the path to the data and run
%          the script (no arguments).
%
% Produces: 'characterization_combined.pdf' (+ PNG) in ../pngs (R-022).
%
...
for folderIdx = 1:1%length(subfolders)
    ...
    for fileIdx = 1:length(csvFiles)
        ...
        try
            ...
            %% Save Figure
            % Save in JournalScripts/pngs/ alongside the other journal figures.
            scriptDir_cd = fileparts(mfilename('fullpath'));
            pngsDir_cd = fullfile(scriptDir_cd, '..', 'pngs');
            if ~exist(pngsDir_cd, 'dir'); mkdir(pngsDir_cd); end
...
% authored width/height are scaled together to target 85% of the 522 pt
...
% ---- SUBPLOT 1: Current ----
ax1 = nexttile;
plot(timeTotal_days, currentTotal, '-', 'Color', darkBlue, 'LineWidth', 0.8);
grid on; box on;
...
% ---- SUBPLOT 2: Voltage ----
ax2 = nexttile;
plot(timeTotal_days, voltageTotal, '-', 'Color', darkBlue, 'LineWidth', 0.8);
grid on; box on;
...
vMin = nanmin(voltageTotal);
vMax = nanmax(voltageTotal);
...
plot(timeTotal_days, chamberTempTotal, '-', 'Color', red,      'LineWidth', 0.8, 'DisplayName', 'Chamber');
plot(timeTotal_days, cellTempTotal,    '-', 'Color', darkBlue, 'LineWidth', 0.8, 'DisplayName', 'Cell');
hold off;
grid on; box on;
% Switched from 'latex' to 'tex' interpreter so FontName 'Times New Roman' is applied (rule R-019).
ylabel('Temperature {\it T} [°C]', 'Interpreter', 'tex', 'FontSize', PUB_FONTSIZE, 'FontName', 'Times New Roman');
xlabel('Time {\it k} [d]',          'Interpreter', 'tex', 'FontSize', PUB_FONTSIZE, 'FontName', 'Times New Roman');
% Put axes ticks/grid/box on top so background patches added later cannot cover them.
set(ax3, 'FontName', 'Times New Roman', 'FontSize', PUB_FONTSIZE, ...
    'LabelFontSizeMultiplier', 1, 'TitleFontSizeMultiplier', 1, ...
    'TickLabelInterpreter', 'tex', 'TickDir', 'out', 'Layer', 'top');
% Set y-limits with headroom
tMin = nanmin([cellTempTotal; chamberTempTotal]);
tMax = nanmax([cellTempTotal; chamberTempTotal]);
yLim3 = [tMin - 1, tMax + 1];
ylim(yLim3);
xlim([0 timeTotal_days(end)]);
...
hold(ax2, 'on');
hold(ax3, 'on');
plot(ax1, timeTotal_days, currentTotal, '-', 'Color', darkBlue, 'LineWidth', 0.8, 'HandleVisibility', 'off');
plot(ax2, timeTotal_days, voltageTotal, '-', 'Color', darkBlue, 'LineWidth', 0.8, 'HandleVisibility', 'off');
...
chamberRed = red;
hChamber = plot(ax3, timeTotal_days, chamberTempTotal, '-', 'Color', chamberRed, 'LineWidth', 0.8, 'DisplayName', 'Chamber');
hCell    = plot(ax3, timeTotal_days, cellTempTotal,    '-', 'Color', darkBlue,   'LineWidth', 0.8, 'DisplayName', 'Cell');
...
% Save into the repo-level pngs/ folder (one level up from matlab_scripts/).
% Using a path relative to this script keeps the export portable across machines.
scriptDir = fileparts(mfilename('fullpath'));
saveDir   = fullfile(scriptDir, '..', 'pngs');
if ~exist(saveDir, 'dir')
    mkdir(saveDir);  % Create the output folder on first run
end
